...
parser.add_argument(
    "--minqual",
    help="Minimum average quality for a read to pass. It is computed over the fastq Phred-scores of each read",
    type=int,
    default=30,
)
parser.add_argument(
    "--maxsnps",
    help="Maximum edit distance on the alignment for a read to pass. It is computed over NM value from the BAM)",
    type=float,
    default=1.0,
)
...
for read in samfile.fetch():
    ...
    if (
        ...
        and (alignment_len >= args.minlen)
        and (snps_rate <= args.maxsnps)
        and (meanqualities >= args.minqual)
        and (refname not in to_exclude)
        and (readname not in reads_to_exclude)
    ):
